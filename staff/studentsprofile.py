    ...
    if staff_role == 'Admin':
        ...
        if isinstance(staff_departments, list):
            ...
            if not depts:
                ...
                or_conditions = [
                    {'dept': {'$regex': f'^{re.escape(dept)}$', '$options': 'i'}, 'collegename': {'$regex': f'^{re.escape(staff_college)}$', '$options': 'i'}}
                    for dept in depts
                ]
            ...
            college_name = re.escape(staff_college)
            query = {'dept': {'$regex': f'^{dept_name}$', '$options': 'i'}, 'collegename': {'$regex': f'^{college_name}$', '$options': 'i'}}
...
def get_cached_students(staff_id, staff_role, staff_college, staff_departments, force_refresh=False):
    """Get students from cache if available, otherwise from database"""
    # Create a cache key based on staff details
    if isinstance(staff_departments, list):
        dept_key = ",".join(sorted(staff_departments))
    else:
        dept_key = str(staff_departments)
        
    cache_key = f"students:{staff_id}:{staff_role}:{staff_college}:{dept_key}"
    
    # Force refresh if requested
    if force_refresh:
        cache.delete(cache_key)
        logger.info(f"Cache cleared for {staff_id} due to refresh request")
    
    # Try to get from cache
    cached_data = cache.get(cache_key)
    if cached_data:
        logger.info(f"Retrieved student data from cache for {staff_id}")
        return cached_data
    ...
    logger.info(f"Stored student data in cache for {staff_id}")
